...
package org.openmrs.module.hivtestingservices.reporting.definition.evaluator;
...
import org.openmrs.module.hivtestingservices.query.patientContact.service.PatientContactQueryService;
...
import org.openmrs.module.hivtestingservices.reporting.data.patientContact.service.PatientContactDataService;
...
public class PatientContactDataSetEvaluator implements DataSetEvaluator {
    ...
    public DataSet evaluate(DataSetDefinition dataSetDefinition, EvaluationContext context) throws EvaluationException {
        ...
        // Construct a PatientContactEvaluationContext based on the patientContact filter
        PatientContactIdSet r = null;
        if (dsd.getRowFilters() != null) {
            for (Mapped<? extends PatientContactQuery> q : dsd.getRowFilters()) {
                PatientContactIdSet s = Context.getService(PatientContactQueryService.class).evaluate(q, context);
                r = QueryUtil.intersectNonNull(r, s);
            }
        }
        if (r == null) {
            r = Context.getService(PatientContactQueryService.class).evaluate(new AllPatientContactQuery(), context);
        }
        PatientContactEvaluationContext vec = new PatientContactEvaluationContext(context, r);
        ...
        for (RowPerObjectColumnDefinition cd : dsd.getColumnDefinitions()) {
            ...
            MappedData<? extends PatientContactDataDefinition> dataDef = (MappedData<? extends PatientContactDataDefinition>) cd.getDataDefinition();
            EvaluatedPatientContactData data = Context.getService(PatientContactDataService.class).evaluate(dataDef, vec);
            ...
            sw.stop();
            if (log.isDebugEnabled()) {
                log.debug("Added patientContact column: " + sw.toString());
            }
