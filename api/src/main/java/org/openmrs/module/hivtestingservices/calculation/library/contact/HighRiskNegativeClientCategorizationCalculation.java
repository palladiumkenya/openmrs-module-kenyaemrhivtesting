...
public class HighRiskNegativeClientCategorizationCalculation extends AbstractPatientCalculation implements PatientFlagCalculation {
     ...
     * 2. Tested negative HTS and MCH modules: Not Enrolled
     ...
     #Criteria
     * 3. In discordant relationship :      SOURCE: HTS initial/Retest
                ...
                if(lastHtsEnc != null) {
                    ...
                    if (patientHasNegativeTestResult) {
                       // 3. In discordant relationship
                        ...
                        // 4.1 Multiple sexual partners  (Patient contact)
                        ...
                        if (!patientContacts.isEmpty()) {
                            if(patientContacts.size() > 1){
                                List<PatientContact> multiplePartners = new ArrayList<PatientContact>();

                                  for (PatientContact pc : patientContacts) {
                                         if (pc.getVoided().equals(false)) {
                                             if (pc.getRelationType() != null && (pc.getRelationType().equals(5617) || pc.getRelationType().equals(163565) || pc.getRelationType().equals(162221))){
                                                 multiplePartners.add(pc);
                                             }
                                          }
                                       }
                                if(multiplePartners.size() > 1){
                                    result = true;
                                }
                            }
                        }
